...
func runServer() {
	...
	h := handler.NewRoomHandler(lm, authMiddleware)
	// 处理直播的消息回调
	r.GET("/handlePushStreamEventCallback", gin.WrapF(h.HandlePushStreamEventCallback))
	...
	r.POST("/api/v1/live/verifyAuthToken", gin.WrapF(h.VerifyAuthToken))
	...
	v1 := r.Group("/api/v1")
	{
		...
		}())
		{
			...
			// 生成桌面工具链接地址
			live.POST("/getLiveJumpUrl", gin.WrapF(h.GetLiveJumpUrl))
