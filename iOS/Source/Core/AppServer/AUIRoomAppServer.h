...
+ (void)requestWithPath:(NSString *)path bodyDic:(NSDictionary *)bodyDic completionHandler:(void (^)(NSURLResponse *response, id responseObject,  NSError * error))completed;

+ (void)fetchToken:(void(^)(NSDictionary * _Nullable data, NSError * _Nullable error))completed;

+ (void)createLive:(NSInteger)mode title:(NSString *)title notice:(NSString * _Nullable)notice extend:(NSDictionary * _Nullable)extend completed:(void(^)(AUIRoomLiveInfoModel * _Nullable model, NSError * _Nullable error))completed;
