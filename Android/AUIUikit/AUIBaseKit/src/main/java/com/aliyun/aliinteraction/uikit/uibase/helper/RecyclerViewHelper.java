...
public class RecyclerViewHelper<T> {
     ...
     * @param addedData 新增的数据源
     */
    public void addDataWithoutAnimation(List<T> addedData) {
        if (CollectionUtil.isNotEmpty(addedData)) {
            dataList.addAll(addedData);
            adapter.notifyDataSetChanged();
        }
     ...
     * @param position 待刷新的索引值
     */
    public void updateData(int position, @Nullable Object payload) {
        if (position >= 0 && position < dataList.size()) {
    ...
    private class Adapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position, @NonNull List<Object> payloads) {
            ...
            // 通过渲染器回调, 将真实的数据回调抛出去
            holderRenderer.render(RecyclerViewHelper.this, holder, model, position, getItemCount(), payloads);
        }
