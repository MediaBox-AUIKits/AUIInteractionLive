...
var _ storage.StorageAPI = (*GormDb)(nil)
...
func (g *GormDb) GetRoomList(pageSize int, pageNum int) ([]string, error) {
	...
	if err := g.db.Order("created_at desc").Limit(pageSize).Offset(offset).Find(&rooms).Error; err != nil {
		return nil, fmt.Errorf("limit failed. %w.\n", err)
	}
	var ids []string
...
func (g *GormDb) GetRoomByMeetingId(meetingId string) (*models.RoomInfo, error) {
	var r models.RoomInfo
	rst := g.db.Where("meeting_id = ?", meetingId).First(&r)
	if rst.Error != nil {
...
func (g *GormDb) UpdateRoomStatus(id string, r *models.RoomInfo) error {
