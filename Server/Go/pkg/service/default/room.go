...
func (l *LiveRoomManager) UpdateRoom(id string, title, notice, extends string) (*models.RoomInfo, error) {
	...
	if err != nil {
		log.Printf("UpdateRoom error. id:%s, err:%s", id, err.Error())
		return nil, err
	}
	return record, nil
}
...
func (l *LiveRoomManager) StartLive(id string, role string) (*models.RoomInfo, error) {
	...
	err = l.sa.UpdateRoomStatus(id, record)
	if err != nil {
		log.Printf("StartLive error. id:%s, err:%s", id, err.Error())
		return nil, err
	}
...
func (l *LiveRoomManager) PauseLive(id string, role string) (*models.RoomInfo, error) {
	...
	err = l.sa.UpdateRoomStatus(id, record)
	if err != nil {
		log.Printf("PauseLive error. id:%s, err:%s", id, err.Error())
		return nil, err
	}
...
func (l *LiveRoomManager) StopLive(id string, role string) (*models.RoomInfo, error) {
	...
	err = l.sa.UpdateRoomStatus(id, record)
	if err != nil {
		log.Printf("StopLive error. id:%s, err:%s", id, err.Error())
		return nil, err
	}
...
func (l *LiveRoomManager) DeleteRoom(id string) (*models.RoomInfo, error) {
	...
	if err != nil {
		log.Printf("DeleteRoom error. id:%s, err:%s", id, err.Error())
		return nil, err
	}
...
func (l *LiveRoomManager) HandleLivePushStreamCallbackEvent(liveRoomManagerEvent *models.LivePushStreamEvent) (bool, error) {

	signContent := fmt.Sprintf("%s|%s|%s", l.appConfig.LiveStreamConfig.PushUrl, liveRoomManagerEvent.LiveTimestamp, l.appConfig.LiveCallbackConfig.AuthKey)
	sum := md5sum(signContent)
	log.Printf("HandleLivePushStreamCallbackEvent signContent:%s, sum:%s, requestSum:%s", signContent, sum, liveRoomManagerEvent.LiveSignature)

	if sum != liveRoomManagerEvent.LiveSignature {
		log.Println("HandleLivePushStreamCallbackEvent sign invalid.")
		return false, fmt.Errorf("HandleLivePushStreamCallbackEvent sign invalid")
	}

	r, err := l.sa.GetRoom(liveRoomManagerEvent.Id)
	if err != nil {
		if strings.Contains(err.Error(), "record not found") {
			// 针对Rtc推流，拿到其rtc的roomId.
			infoSlice := strings.Split(liveRoomManagerEvent.Id, "_")
			if len(infoSlice) >= 3 {
				meetingId := infoSlice[1]
				r, err = l.sa.GetRoomByMeetingId(meetingId)
				if err != nil {
					log.Printf("HandleLivePushStreamCallbackEvent error. meetingId:%s, error:%s.\n", meetingId, err.Error())
					return false, err
				}
			}
		} else {
			log.Printf("HandleLivePushStreamCallbackEvent error. streamId:%s, error:%s", liveRoomManagerEvent.Id, err.Error())
			return false, err
		}
	}

	if r == nil {
		log.Printf("HandleLivePushStreamCallbackEvent room not found. roomId:%s.\n", liveRoomManagerEvent.Id)
		return true, nil
	}

	if models.PUBLIC == liveRoomManagerEvent.Action {
		// 正常推流
		if r.Status == models.LiveStatusOff || r.Status == models.LiveStatusPrepare {
			log.Printf("HandleLivePushStreamCallbackEvent callback status:%s, but db status:%d.\n", models.PUBLIC, r.Status)
			l.StartLive(r.ID, r.AnchorId)
		}
	} else if models.PUBLIC_DONE == liveRoomManagerEvent.Action {
		// 推流断开
		if r.Status == models.LiveStatusOn {
			log.Printf("HandleLivePushStreamCallbackEvent callback status:%s, but db status:%d.\n", models.PUBLIC_DONE, r.Status)
			l.PauseLive(r.ID, r.AnchorId)
		}
	}
...
var jwtSecret = []byte("323assa2323.dqe223b434")
...
func (l *LiveRoomManager) GetLiveJumpUrl(in *models.JumpUrlRequest, appServerHost string) (string, error) {

	myClaims := &MyClaims{
		UserId:    in.UserId,
		LiveId:    in.LiveId,
		UserName:  in.UserName,
		AppServer: appServerHost,
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(1 * time.Hour)),
		},
	}
	token, err := jwt.NewWithClaims(jwt.SigningMethodHS256, myClaims).SignedString(jwtSecret)
	if err != nil {
		log.Printf("GetLiveJumpUrl error. in:%v, error:%s.\n", in, err.Error())
		return "", err
	}

	url := fmt.Sprintf("auipusher://page/live-room?app_server=%s&token=%s&user_id=%s&user_name=%s&live_id=%s",
		url.QueryEscape(appServerHost), token, url.QueryEscape(in.UserId), url.QueryEscape(in.UserName), url.QueryEscape(in.LiveId))
	log.Printf("GetLiveJumpUrl url:%s.\n", url)
	return url, nil
}

func (l *LiveRoomManager) VerifyAuthToken(in *models.AuthTokenRequest, host string) error {

	token, err := jwt.ParseWithClaims(in.Token, &MyClaims{}, func(token *jwt.Token) (interface{}, error) {
		return jwtSecret, nil
	})
	if err != nil {
		log.Printf("VerifyAuthToken error. in:%v, error:%s.\n", in, err.Error())
		return err
	}
	if claims, ok := token.Claims.(*MyClaims); ok && token.Valid {
		log.Printf("claims:%v.\n", claims)
		if claims.AppServer != host {
			log.Printf("VerifyAuthToken invalid. in:%v, error:appserver not matched.\n", in)
			return fmt.Errorf("appserver not matched")
		}
		if claims.LiveId != in.LiveId {
			log.Printf("VerifyAuthToken invalid. in:%v, error:LiveId not matched.\n", in)
			return fmt.Errorf("LiveId not matched")
		}
		if claims.UserId != in.UserId {
			log.Printf("VerifyAuthToken invalid. in:%v, error:UserId not matched.\n", in)
			return fmt.Errorf("UserId not matched")
		}
		if claims.UserName != in.UserName {
			log.Printf("VerifyAuthToken invalid. in:%v, error:UserName not matched.\n", in)
			return fmt.Errorf("UserName not matched")
		}
		return nil
	} else {
		log.Printf("VerifyAuthToken valid error. in:%v, error:%s.\n", in, err.Error())
		return err
	}
}
