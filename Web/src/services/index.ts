...
class Services {
  ...
  public checkLogin() {
    return new Promise((resolve, reject) => {
      ...
      if (cookieObj[AuiUserNameCookieKey] && cookieObj[AuiUserIdCookieKey] && cookieObj[AuiUserToken]) {
        ...
        this.userNick = cookieObj[AuiUserNameCookieKey];
  ...
  private setLoginCookie(expireStr?: string) {
    ...
    setCookie(AuiUserNameCookieKey, this.userNick);
  ...
  public async getToken(im_server: string[], role?: string, userId?: string) {
    let device_id;
    if(localStorage.getItem(IMDeviceIdStorageKey)) {
   ...
   */
  public async getMeetingInfo(roomId: string) {
    try {
   ...
   */
  public async updateMeetingInfo(roomId: string, members: ISpectatorInfo[]) {
    try {
